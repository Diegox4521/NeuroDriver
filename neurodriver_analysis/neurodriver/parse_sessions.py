...
def parse_session(path):
    ...
    # The filename is the Qualtrics ResponseId, the canonical key for joining
    # with the survey exports. The in-browser participantId is unreliable: in 3
    # of 69 sessions it carries a stale value copied from a previous student,
    # which would collide records and break the survey merge. Prefer the
    # filename and only fall back to the embedded id when the stem is missing.
    pid = Path(path).stem or s.get('participantId')
